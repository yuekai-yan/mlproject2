...
def train(model,
    ...
    test_metrics = trainer.test(model, dataloaders=test_datalodaer, verbose=False)
    print(test_metrics)
    ...
    #         _outputs = torch.softmax(outputs, dim=1)  # 对通道维度应用 softmax
    #         _outputs = _outputs[:, 1:2, :, :]  # 只保留类别 1 的概率，形状变为 [8, 1, 384, 384]
    #         # print(f'_outputs.shape={_outputs.shape}, labels.shape={labels.shape}')
    #         loss = loss_func(_outputs, labels)
    ...
    #                 _outputs = _outputs[:, 1:2, :, :]  # 只保留类别 1 的概率，形状变为 [8, 1, 384, 384]
    #                 # print(f'_outputs.shape={_outputs.shape}, labels.shape={labels.shape}')
    #                 loss_validation += loss_func(_outputs, labels)
